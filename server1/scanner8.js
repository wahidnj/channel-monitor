...
const directoryPath = path.join(__dirname, "image8");
...
async function scanner() {
    // fs.readdir(directoryPath, async function (err, files) {
        ...
        try {
            ...
            await Promise.all(files.filter((item, idx) => idx < 150).map(async (filename) => {
                    const hash1 = await imghash.hash(`./image8/${filename}`);
                    ...
                    if(hash1){
                        ...
                        const filePath= `./image8/${result.image}`;
                        if(result){
